...
在輸入保險資料時，很常需要手動把西元出生年月日改成民國出生年月日，另外也要手動把身分證字號複製到保險網頁，有很高機率造成誤植。這套程式將把輸入保險資料變成是一直按 enter 鍵確認，不僅可節省將近 5 倍的時間，更大大減少出錯機率。另外加退保完成後可以將加退保資料下載進行比對，免除人工比對數十筆加退保資料，減少出錯機率。
